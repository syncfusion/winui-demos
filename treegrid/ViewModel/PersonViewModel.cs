    ...
    public class PersonViewModel
    {
        ...
        public ObservableCollection<Person> CreateGenericPersonData(int count, int maxGenerations)
        {
            ...
            if (count > 0)
            {
                ...
                {
                    var contactNo = 9991121234 + random.Next(10000);
                    var lastname = names2[random.Next(names2.GetLength(0))];
                    personList.Add(new Person(names1[random.Next(names1.GetLength(0))], lastname, new DateTime(random.Next(2008, 2012), random.Next(1, 12), random.Next(1, 28)), 30000d + random.Next(9) * 10000, city[random.Next(city.GetLength(0))], contactNo.ToString(CultureInfo.CurrentCulture), this.CreateChildList(count, (int)Math.Max(0, maxGenerations - 1))));
                }
            }
            return personList;
        }
