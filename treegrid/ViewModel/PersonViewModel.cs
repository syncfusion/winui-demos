    ...
    public class PersonViewModel : IDisposable
    {
        ...
        public ObservableCollection<Person> CreateGenericPersonData(int count, int maxGenerations)
        {
            ...
            if (count > 0)
            {
                for (int i = 0; i < count; i++)
                {
                    ...
                    personList.Add(new Person(names1[random.Next(names1.GetLength(0))], lastname, new DateTimeOffset(new DateTime(random.Next(2008, 2012), random.Next(1, 12), random.Next(1, 28))),new DateTimeOffset(new DateTime(random.Next(1975, 1985), random.Next(1, 12), random.Next(1, 28), random.Next(1, 24), random.Next(1, 60), random.Next(1, 60))), 30000d + random.Next(9) * 10000, city[random.Next(city.GetLength(0))], contactNo, emailId, this.CreateChildList(count, (int)Math.Max(0, maxGenerations - 1))));
                }
            }
            return personList;
        }
        ...
        private void BestFit(object param)
        {
            ...
            if (contextMenuInfo == null)
                return;            
            var column = contextMenuInfo.Column;
            column.ColumnWidthMode = Syncfusion.UI.Xaml.Grids.ColumnWidthMode.Auto;
        }
        ...
        private void AddAbove(object param)
        {
            ...
            var newItem = new Person() { FirstName = names1[random.Next(names1.GetLength(0))], Children = new System.Collections.ObjectModel.ObservableCollection<Person>(), LastName = names2[random.Next(names2.GetLength(0))], DOJ = new DateTimeOffset(new DateTime(random.Next(2008, 2012), random.Next(1, 12), random.Next(1, 28))),CheckInTime = new DateTimeOffset(new DateTime(random.Next(1975, 1985), random.Next(1, 12), random.Next(1, 28), random.Next(1, 24), random.Next(1, 60), random.Next(1, 60))), Salary = 30000d + random.Next(9) * 10000, City = city[random.Next(city.GetLength(0))], ContactNumber = contactNos[random.Next(0, contactNos.Count() - 1)], EMail = names[random.Next(names.Length - 1)] + "@" + mail[random.Next(0, mail.Count() - 1)] };
