...
namespace Syncfusion.ChatDemos.WinUI
{
    internal class ComposeViewModel : INotifyPropertyChanged
    {
        ...
        private ObservableCollection<object> suggestion;
        ...
        public ComposeViewModel()
        {
            ...
            this.Suggestion = new ObservableCollection<object>();
        ...
        internal void InitAI()
        {
            ...
            this.Chats.Add(new AIMessage
            {
                Author = new Author { Name = "Bot", ContentTemplate = AIIcon },
                DateTime = DateTime.Now,
                Solution = service.Question
            });
            this.Suggestion = new ObservableCollection<object>(service.Options);
        ...
        private async void Send(string message)
        {
            ...
            Suggestion = new ObservableCollection<object>(service.Options);
            if (!isStopResponding)
            {
  ...
  ""required"": [
    ...
    ""Suggestions""
        ...
        internal async void ResponseToolbarItemClicked(ResponseToolbarItemClickedEventArgs e)
        {
            ...
            {
                await Task.Delay(2000);
                string response = service.Question;
            ...
            else if (e.Item.ItemType == ToolBarItemType.Copy)
            {
                var aiMessage = e.ChatItem.DataContext as AIMessage;
    ...
    public class Booking
    {
        ...
        public Booking()
        {
            Countries = new List<Country>()
            {
                ...
                new Country() { Name = "Canada", Image = new Uri("ms-appx:///ChatDemos/Assets/Canada.png"),
                Cities = new List<City>
                    {
                        new City{
                            Name = "Toronto",
            ...
            Questions = new Dictionary<string, GetInput>(
                new List<KeyValuePair<string, GetInput>>() {
                    new KeyValuePair<string, GetInput>(
                        nameof(Name),
                        new GetInput {
                            Question = "Whats your name?",
                            Suggestions = new List<object>() { "A", "B" }
                        }),
                    new KeyValuePair<string, GetInput>(
                        nameof(Departure),
                        new GetInput {
                            Question = "When are you planning to depart?",
                            Suggestions = new List<object>{ typeof(DateOnly).ToString() }
                        }),
                    new KeyValuePair<string, GetInput>(
                        nameof(NumberOfDays),
                        new GetInput {
                            Question = "How many days would you like to stay?",
                            Suggestions = new List<object>{ "2", "4", "8", "16", typeof(int).ToString() }
                        }),
                    new KeyValuePair<string, GetInput>(
                        nameof(Adults),
                        new GetInput {
                            Question = "How many adults are traveling?",
                            Suggestions = new List<object>{ "1", "2", "3", typeof(int).ToString() }
                        }),
                    new KeyValuePair<string, GetInput>(
                        nameof(Children),
                        new GetInput {
                    ...
                    new KeyValuePair<string, GetInput>(
                        ...
                        new GetInput {
                            ...
                            Suggestions = new List<object>{ typeof(Class).ToString() }
                        }),
                    new KeyValuePair<string, GetInput>(
                        nameof(Airline),
                        new GetInput {
                            Question = "Do you have a preferred airline?",
                            Suggestions = new List<object>{ typeof(Airline).ToString() }
                        }),
                    new KeyValuePair<string, GetInput>(
                        nameof(Return),
                        new GetInput {
                            Question = "Will this be a round-trip booking?",
                            Suggestions = new List<object>{ "Yes", "No" }
                        }),
                    new KeyValuePair<string, GetInput>(
                        nameof(Country),
                        new GetInput {
                            Question = $"Welcome {Name}! \n\nI’m here to help you plan your perfect trip. " +
                            "Whether you're booking flights, hotels, or need travel recommendations, I’ve got you covered. " +
                            "Let’s get started\n\nwhere would you like to go?",
                            Suggestions = new List<object> { Countries }
                        }),
                    new KeyValuePair<string, GetInput>(
                        nameof(City),
                        new GetInput {
                            Question = $"Which city in {Country} would you like to travel to?",
                            Suggestions = new List<object>()
                        }),
                    new KeyValuePair<string, GetInput>(
                        nameof(Hotel),
                        new GetInput {
                            Question = $"Please select a hotel in {City}?",
                            Suggestions = new List<object>()
                        }),
                });
        }
        ...
        private void OnSelectedCountryChanged()
        {
            if (Country != null)
            {
                ...
                Questions[nameof(City)].Suggestions = new List<object> { Cities };
            }
            else
            {
                Cities = new List<City>();
                Questions[nameof(City)].Suggestions = new List<object>();
                City = null;
            }
        }
        ...
        private void OnSelectedCityChanged()
        {
            ...
            {
                Hotels = City.Hotels;
                Questions[nameof(Hotel)].Suggestions = new List<object> { Hotels };
            }
            else
            {
                Hotels = new List<Hotel>();
                Questions[nameof(Hotel)].Suggestions = new List<object>();
                Hotel = null;
            }
        }
        ...
        public bool TrySetValue(string prop, string value, out string error)
        {
            switch (prop)
            {
                ...
                case nameof(Departure):
                    if (DateOnly.TryParse(value, out DateOnly departure))
                    {
                        if (departure.ToDateTime(default) < DateTime.Now)
                        {
                ...
                case nameof(NumberOfDays):
                    ...
                    break;
                case nameof(Adults):
                    if (int.TryParse(value, out int adults))
                    {
                        if (adults < 1)
                        {
                ...
                case nameof(Children):
                    ...
                    break;
                case nameof(Class):
                    if (Enum.TryParse<Class>(value, out Class travelClass))
                    {
                        Class = travelClass;
                    }
                    else
                    ...
                    break;
                case nameof(Airline):
                    if (Enum.TryParse<Airline>(value, out Airline airline))
                    {
                        Airline = airline;
                    }
                    else
                ...
                case nameof(Return):
                    ...
                    break;
                case nameof(Country):
                    Country = Countries.FirstOrDefault(c => c.Name.ToLower() == value.ToLower());
                    break;
                case nameof(City):
    ...
    public class SuggestionUISelector : SuggestionTemplateSelector
    {
        ...
        {
            if(item is string template)
            {
                if(template == typeof(DateOnly).ToString())
                {
    ...
    public class SelectionBehavior : ISuggestionSelectionBehavior
    {
        ...
        {
            object newValue = e.AddedItems.FirstOrDefault();
            if (newValue is Country country)
            {
                newValue = country.Name;
            }
            else if (newValue is City city)
            {
                newValue = city.Name;
            }
            else if (newValue is Hotel hotel)
            {
                newValue = hotel.Name;
            }
        ...
        {
            if (obj is CalendarDatePicker dp)
            {
                dp.DateChanged += Dp_DateChanged;
            }
            else if (obj is ComboBox cb)
            {
                var type = Type.GetType(cb.DataContext as string);
                cb.ItemsSource = Enum.GetValues(type).Cast<object>().ToList();
                cb.PlaceholderText = "Select";
                cb.SelectionChanged += ComboBox_SelectionChanged;
            }
            else if(obj is ListBox lb)
            {
                lb.ItemsSource = lb.DataContext;
                lb.SelectionChanged += ComboBox_SelectionChanged;
            }
            else if(obj is NumberBox nb)
            {
                nb.Value = 2;
        ...
        private void Dp_DateChanged(CalendarDatePicker sender, CalendarDatePickerDateChangedEventArgs e)
        {
            var args = new SuggestionClickedEventArgs(string.Format("{0:d}", e.NewDate), sender as UIElement);
            RaiseEvent(sender, args);
        }

        private void Nb_ValueChanged(NumberBox sender, NumberBoxValueChangedEventArgs e)
        {
            var args = new SuggestionClickedEventArgs(e.NewValue, sender as UIElement);
            RaiseEvent(sender, args);
        }
