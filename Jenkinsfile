...
timestamps
{
 timeout(time: 7200000, unit: 'MILLISECONDS') 
 {
    ...
    try
    {	
	  checkout([$class: 'GitSCM', branches: [[name: '*/$githubSourceBranch']], doGenerateSubmoduleConfigurations: false, extensions: [[$class: 'RelativeTargetDirectory', relativeTargetDir: 'SB-winui']], submoduleCfg: [], userRemoteConfigs: [[credentialsId: env.GiteaCredentialID, url: 'https://gitea.syncfusion.com/essential-studio/samplebrowser-winui.git']]])
	  
	  checkout([$class: 'GitSCM', branches: [[name: 'development']], doGenerateSubmoduleConfigurations: false, extensions: [[$class: 'RelativeTargetDirectory', relativeTargetDir: 'xaml-cake-plugin']], submoduleCfg: [], userRemoteConfigs: [[credentialsId: env.GiteaCredentialID, url: 'https://gitea.syncfusion.com/essential-studio/xaml-cake-plugin.git']]])
    }
    catch(Exception e)
    {
...
if(currentBuild.result != 'FAILURE' && env.publishBranch.contains(githubSourceBranch))
{
	...
	try
	{
	  //method to get release notes content
       ...
       def branchCommit = '"'+'https://gitea.syncfusion.com/api/v1/repos/essential-studio/SB-winui/pulls/'+env.pullRequestId+'/commits'
